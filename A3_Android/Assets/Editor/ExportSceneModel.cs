...
namespace MgaEditor
{
    ...
    public class EditorObjExporter : ScriptableObject
    {
        ...
        private static string MeshToString(MeshFilter mf, Dictionary<string, ObjMaterial> materialList)
        {
            ...
            if (mf.name == "7")
                sb.Append("g ").Append("mesh7").Append("\n");
            else
            ...
            foreach (Vector3 lv in m.normals)
            {
                ...
                sb.Append(string.Format("vn {0} {1} {2}\n", Math.Abs(wv.x) < 1e-3 ? 0 : -wv.x, Math.Abs(wv.y) < 1e-3 ? 0 : wv.y, Math.Abs(wv.z) < 1e-3 ? 0 : wv.z));
            }
            sb.Append("\n");
		...
		/*
		[MenuItem("Assets/[CrossMono]Export Objects To Model")]
		...
		*/
		...
		[MenuItem("Assets/[CrossMono]/map导出选中物体的白膜")]
        static void ExportWholeSelectionToSingle()
        {
            //if (!CreateTargetFolder())
            //    return;

			string path = EditorUtility.SaveFolderPanel("Export Scene", "", "");
			if (path == "")
				return;
            ...
            if (selection.Length == 0)
            {
				EditorUtility.DisplayDialog("Export Scene", "Please select one or more target objects", "OK");
                return;
            }
            ...
            if (exportedObjects > 0)
            {
				...
				MeshesToFile(mf, path, filename);


				EditorUtility.DisplayDialog("Export Scene", "Completed", "OK");
            }
            else
				EditorUtility.DisplayDialog("Export Scene", "No mesh detected!", "OK");
        }


		/*
		[MenuItem("Assets/Export each selected to single OBJ")]
