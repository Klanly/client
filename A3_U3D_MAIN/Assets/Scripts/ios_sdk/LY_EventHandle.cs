...
public class LY_EventHandle : MonoBehaviour
{
    ...
    [DllImportAttribute("__Internal")]
    public static extern void AppStoreBuy(string price, string roleid, string cp_trade_no, string servername, string waresId,
        string waresName, string cpPrivateInfo, string desc, string rolename, string rolelevel, string roleCreateTime);

    [DllImportAttribute("__Internal")] //创角 上报
    public static extern void AppsFlyerCreateRole(string zoneId, string cId, string roleName, string roleLevel, string zoneName, string puid, string roleCreateTime);

    [DllImportAttribute("__Internal")] //升级 上报
    public static extern void AppsFlyerLevelUp(string zoneId, string cId, string roleName, string roleLevel, string zoneName, string puid, string roleCreateTime);


    [DllImportAttribute("__Internal")] //登录 上报
    public static extern void AppsFlyerLogin(string zoneId, string cId , string roleName, string roleLevel, string zoneName, string puid, string roleCreateTime);

    [DllImportAttribute("__Internal")] //退出 上报
    public static extern void AppsFlyerLoginOut(string zoneId, string cId, string roleName, string roleLevel, string zoneName, string puid, string roleCreateTime);


    //充值档位【 Non60 : 月卡 ，  其他钻石档位 都是 对应的价钱   30 ： 30   150 ： 150 】

    //end

    static public LY_EventHandle _instance = null;
    ...
    public void getOrderID() //向公司后台请求订单号
    {
        ...
        m_keys.Add("roleid"); m_values.Add(m_buy_date["roleId"]._str);
        m_keys.Add("orderTitle"); m_values.Add(m_buy_date["productName"]._str);
        m_keys.Add("orderDesc"); m_values.Add(m_buy_date["productDesc"]._str);
        m_keys.Add("productId"); m_values.Add(m_buy_date["productShopId"]._str);
    ...
    private void on_QW_Pay(bool timeout, WWW www)
    {
        ...
        if (res == CheckHttpRes.CHR_success)
        {
            try
            {
                ...
                string ymOrderID = togame_data["ymorder"];

                Debug.Log( "-- SDKPay -- 获取到的订单ID: " + cpOrderID);


                StartBuyToApp(cpOrderID, ymOrderID);
    ...
    public void sdk_enterGame(Variant v)
    {
        AppsFlyerLogin(v["roleServerId"]._str,
            v["roleId"]._str,
            v["roleName"]._str,
            v["roleLevel"]._str,
            v["roleServerName"]._str,
            v["gpuid"]._str,
            v["roleEnterTime"]._str);
    }

    public void sdk_levelUp(Variant v)
    {
        AppsFlyerLevelUp(v["roleServerId"]._str,
         v["roleId"]._str,
            v["roleName"]._str,
            v["roleLevel"]._str,
            v["roleServerName"]._str,
    ...
    public void sdk_createRole(Variant v)
    {
        AppsFlyerCreateRole(v["roleServerId"]._str,
         v["roleId"]._str,
            v["roleName"]._str,
            v["roleLevel"]._str,
            v["roleServerName"]._str,
    ...
    public void sdk_exitPage(Variant v)
    {
        AppsFlyerLoginOut(v["roleServerId"]._str,
         v["roleId"]._str,
            v["roleName"]._str,
            v["roleLevel"]._str,
            v["roleServerName"]._str,
    ...
    {
        showLog("onLoginSuccess", "uid: " + userInfo.uid +  " ,userToken: " + userInfo.token +
            " ,username: " + userInfo.username + "time: " + userInfo.time + ", msg: " + userInfo.errMsg);
        ...
        //m_keys.Add("time"); m_values.Add(userInfo.time);
    ...
    public void StartBuyToApp(string cpOrderID, string ymOrderID)
    {
        AppStoreBuy(m_buy_date["productPrice"]._str,
            m_buy_date["roleId"]._str,
            cpOrderID,
            m_buy_date["serverName"]._str,
            m_buy_date["productShopId"]._str,
            m_buy_date["productName"]._str,
            Globle.curServerD.sids.ToString() + "|" + m_buy_date["productId"]._str,
            m_buy_date["productDesc"]._str,
            m_buy_date["roleName"]._str,
            m_buy_date["roleLevel"]._str,
            m_buy_date["roleCreateTime"]
            );
    }
